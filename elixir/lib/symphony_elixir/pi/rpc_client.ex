defmodule SymphonyElixir.Pi.RpcClient do
  ...
  def configure_turn(%{port: port, response_timeout_ms: timeout_ms}, %{name: session_name} = turn_config)
  ...
  @spec receive_message(session(), timeout(), String.t()) ::
          {:ok, incoming_message(), String.t()} | {:error, term()}
  def receive_message(%{port: port} = session, timeout_ms, pending_line \\ "") do
    receive do
      {^port, {:data, {:eol, chunk}}} ->
        handle_data_line(session, pending_line <> to_string(chunk))
  ...
  defp handle_data_line(_session, data) do
  ...
  defp session_identity_from_response(%{"data" => %{"sessionId" => session_id} = data})
