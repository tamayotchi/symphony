defmodule SymphonyElixir.AgentRunner do
  ...
  def run(issue, worker_update_recipient \\ nil, opts \\ []) do
    ...
    Logger.info("Starting agent run for #{issue_context(issue)} worker_host=#{worker_host_for_log(worker_host)} runtime=#{Config.worker_runtime()}")
  ...
  defp run_on_worker_host(issue, worker_update_recipient, opts, worker_host) do
    Logger.info("Starting worker attempt for #{issue_context(issue)} worker_host=#{worker_host_for_log(worker_host)} runtime=#{Config.worker_runtime()}")
  ...
  defp do_run_worker_turns(runtime_module, run_context, issue, turn_number) do
    %{
      session: session,
      workspace: workspace,
      worker_update_recipient: worker_update_recipient,
    ...
    with {:ok, turn_session} <-
      ...
      case continue_with_issue?(issue, issue_state_fetcher) do
        {:continue, refreshed_issue} when turn_number < max_turns ->
          ...
          do_run_worker_turns(runtime_module, run_context, refreshed_issue, turn_number + 1)
